...
export default function SignInPage() {
  ...
  const submitHandler = async (e) => {
    ...
    if (shown) return;
    setShown(true);
    if (btn.current) btn.current.innerText = "لطفا منتظر بمانید";
    ...
    try {
      ...
      if (response?.status === 201) {
        const token = response.data.token;
        // Update global auth state (also writes to localStorage)
        setToken(token);

        const rolePayload = jwtDecode(token);
        const { redirectTo, courseId } = location.state || {};
        ...
        if (rolePayload.role === "Administrator") {
      ...
      if (status === 400) {
        ...
        console.log(error.message);
        setShown(false);
        if (btn.current) btn.current.innerText = "ورود";
        showToast("خطایی رخ داده است. لطفا دوباره تلاش کنید", "error");
      }
    }
  };
